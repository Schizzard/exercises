...
# searching files by a specific mask are just what you need for these sort of 
# challenges.
...
# Let me show you what I mean by matching the filenames in the unix-shell. For 
# example, * matches everything and *.txt matches all of the files that have 
...
# my solve (first) is not accepted
...
# my solve (second)
def unix_match(filename: str, pattern: str) -> bool:
    pattern_1 = pattern.replace('.','\\.').replace('*','.*').replace('?','.')
    match1 = re.match(pattern_1, filename)
    return True if match1 is not None else False
